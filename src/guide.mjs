...
const extra=[
    ...
    ['이미지 읽기','NovelAI 원본 PNG의 일반 텍스트와 알파 채널 메타데이터를 브라우저에서 읽습니다. 설정 적용은 생성 옵션만 저장하고, 장면 편집은 프롬프트·인물·좌표까지 가져옵니다. 시드는 읽거나 적용하지 않으며 현재 시드 설정을 유지합니다. 스크린샷·변환본·메타데이터 삭제본은 복원할 수 없습니다. 모르는 모델은 직접 선택해야 합니다. 이미지 원본이나 참조용 인코딩, Img2Img 설정은 복원하지 않습니다.'],

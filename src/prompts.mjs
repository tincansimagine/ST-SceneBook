...
- Pick the concrete turning point, exchange, movement or reaction that belongs at the insertion paragraph. Use each paragraph at most once. A single image captures one instant, never a before/after montage. Choose fewer scenes when the remaining candidates repeat the same visual information. Across adjacent scenes vary the framing when it helps reading, without changing the facts.
...
For a revision, retain each existing after and evidence value unless relocation is expressly requested. Modify only the requested visual aspect. User direction: {{direction}}.
DATA={{data}}`;
